...
interface GameWithPedroProps {
  score: number;
  setScore: React.Dispatch<React.SetStateAction<number>>;
}

export default function GameWithPedro({ score, setScore }: GameWithPedroProps) {
  const [currentIdx, setCurrentIdx] = useState<number>(0);
  const [selectedOptionIdx, setSelectedOptionIdx] = useState<number | null>(null);
  const [hasAnswered, setHasAnswered] = useState<boolean>(false);
  const [isGameFinished, setIsGameFinished] = useState<boolean>(false);
  const [highScore, setHighScore] = useState<number>(() => {
    return Number(localStorage.getItem('pedro_high_score') || '0');
  });
  ...
  useEffect(() => {
    ...
    if (isGameFinished && score > highScore) {
      localStorage.setItem('pedro_high_score', String(score));
      setHighScore(score);
    }
  ...
  // Pedro's visual expression calculator
  const getPedroExpression = () => {
    if (!hasAnswered) {
  ...
  const getBadgeDetails = (scorePoints: number) => {
    ...
    if (questionsCorrect >= 9) {
      return {
        ...
        desc: "Մակարդակ՝ Իսպանախոս Փորձագետ: Դուք կատարյալ հասկանում եք անդրադարձ դերանունների նրբությունները և օգնեցիք Պեդրոյին կազմակերպել իր օրը հիանալի կերպով!",
    ...
    return {
      ...
      desc: "Մակարդակ՝ Սկսնակ: Ոչինչ, սա միայն սկիզբն է: Պեդրոն խորհուրդ է տալիս ևս մեկ անգամ կարդալ տեսությունը և նորից փորձել:",
      ...
      {!isGameFinished ? (
          ...
          <div className="bg-white border-2 border-orange-100 rounded-3xl p-4 md:p-6 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="space-y-1">
              ...
              <h3 className="font-display font-bold text-slate-800 text-sm md:text-base">
                Օգնիր Պեդրոյին կողմնորոշվել ճիշտ բայաձևերում
              </h3>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
              ...
              <div className="md:col-span-3 flex flex-col items-center justify-center space-y-2">
                ...
                <div className="text-center">
                  ...
                  <span className="block text-[10px] opacity-60 font-mono">Madrid, España</span>
                </div>
              </div>
        ...
        <div className="bg-white border-2 border-orange-200 rounded-3xl p-6 md:p-10 shadow-xl relative overflow-hidden text-center space-y-8 max-w-2xl mx-auto">
          ...
          <div className="bg-amber-50/50 p-4 rounded-xl border border-amber-200/40 text-left text-xs text-slate-700 italic flex items-start gap-3">
            ...
            <div>
              <strong>Պեդրոյի խոսքը՝</strong> «Ես անչափ ուրախ եմ, որ դուք անցաք իմ օգնությամբ այս դժվարին թեման։ Իսպաներենում անդրադարձ բայերը կարևորագույն դեր ունեն առօրյա կյանքում։ Լավագույնս տիրապետելու համար պրակտիկա բաժնում ինքնուրույն փորձարկեք խոնարհումները։ ¡Hasta la vista, amigo!»
            </div>
          </div>
